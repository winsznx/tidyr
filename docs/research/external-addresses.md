...
| Uniswap SwapRouter02               | `0xfE31F71C1B106EAC32F1a19239c9A9A72DDfB900`      | Yes                | Bytecode contains the no-deadline `exactInput((bytes,address,uint256,uint256))` selector `0xb858183f` and `exactInputSingle(...)` selector `0x04e45aaf`; does **not** contain the old deadline-inclusive `exactInput` selector `0xc04b8d59` — confirms this is genuine SwapRouter02, not the older SwapRouter | Uniswap Monad Deployments; selector presence verified against live bytecode via `eth_getCode` + `cast sig`       |
